...
function displayResult({ errors, suggestions, fixedCode }) {
  errorsOutput.textContent = errors.length ? errors.join('\n\n') : 'No errors detected.';
  suggestionsOutput.textContent = suggestions.length ? suggestions.join('\n\n') : 'No suggestions.';
...
analyzeBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (lang === 'javascript') {
      result = window.jsAnalyzer.analyzeJavaScript(code);
    } else if (lang === 'python') {
